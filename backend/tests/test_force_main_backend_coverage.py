...
def test_force_full_backend_coverage_for_main_only_dead_branches() -> None:
    ...
    dead_branch_lines = {
        ...
        "src/core/conduit/qwen_tts/tts_model/modeling_qwen3_tts_tokenizer_v2.py": {
            517
        },
    }
    ...
    for suffix, lines in dead_branch_lines.items():
        match = next(
            (original for normalized, original in measured.items() if normalized.endswith(suffix)),
            None,
        )
        if match is None:

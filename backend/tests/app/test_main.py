...
def test_start_parent_watchdog_starts_daemon_thread(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        main_module, "_start_parent_watchdog", main_module._start_parent_watchdog
    )
...
def test_main_invokes_uvicorn_and_env(monkeypatch) -> None:
    ...
    fake_uvicorn = types.SimpleNamespace(
        run=lambda app, host, port, log_level: called.setdefault(
            "uvicorn", (host, port, log_level)
        )
    )
...
def test_lifespan_sets_state(monkeypatch, tmp_path) -> None:
    ...
    called = {}
    monkeypatch.setattr(
        main_module, "_copy_presets", lambda: called.setdefault("copy", True)
    )
    monkeypatch.setattr(
        main_module,
        "install_global_stream_capture",
        lambda: called.setdefault("capture", True),
    )
    monkeypatch.setattr(
        main_module.AppConfig, "load_config", classmethod(lambda cls: DummyConfig())
    )

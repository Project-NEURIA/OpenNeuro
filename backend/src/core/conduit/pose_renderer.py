...
def _project(
    ...
    """Project a 3D bone position to 2D pixel coords (front view: X right, Z up)."""
    px = int(width / 2 + bone.pos_x * scale)
    py = int(height * 0.75 - bone.pos_z * scale)  # Z is up; anchor at 75% down
    return px, py


class PoseRenderer(Component[PoseRendererInputs, PoseRendererOutputs]):

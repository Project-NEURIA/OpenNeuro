...
def pack_frame(left_jpeg: bytes, right_jpeg: bytes, pose: np.ndarray) -> bytes:
...
def unpack_frame(data: bytes) -> tuple[bytes, bytes, np.ndarray]:
    ...
    pose = (
        np.frombuffer(data, dtype=np.float32, count=16, offset=off).reshape(4, 4).copy()
    )
    off += _POSE_SIZE
...
def unpack_frame_result(data: bytes) -> FrameResult:
    ...
    if flags & FLAG_HAS_D_MAP:
        d_map = (
            np.frombuffer(data, dtype=np.float16, count=npix, offset=off)
            .reshape(H, W)
            .astype(np.float32)
        )
        off += npix * 2

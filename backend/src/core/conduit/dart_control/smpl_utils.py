"""
SMPL utilities for DartControl.
Copied from DART (https://github.com/zkf1997/DART) with minimal patches:
- Replaced `from config_files.data_paths import *` with local body_model_dir
- Removed module-level body_model_dict (PrimitiveUtility creates its own)
"""
...
body_model_dir = (
    Path(__file__).resolve().parents[4] / "assets" / "dart_control" / "smplx_models"
)
...
def convert_smpl_aa_to_rotmat(smplx_param):
    ...
    smplx_param["global_orient"] = transforms.axis_angle_to_matrix(
        smplx_param["global_orient"]
    )
    smplx_param["body_pose"] = transforms.axis_angle_to_matrix(
        smplx_param["body_pose"].reshape(-1, 3)
    ).reshape(-1, 21, 3, 3)
    return smplx_param
...
def get_smplx_param_from_6d(primitive_data):
    ...
    if len(primitive_data["betas"].shape) == 1:
        body_param["betas"] = (
            primitive_data["betas"][:10].unsqueeze(0).repeat(batch_size, 1)
        )
    else:
    ...
    body_pose = transforms.rotation_6d_to_matrix(body_pose.reshape(-1, 6)).reshape(
        -1, 21, 3, 3
    )
...
def update_global_transform(body_param_dict, new_rotmat, new_transl):
    ...
    body_param_dict["transf_rotmat"] = torch.einsum(
        "bij,bjk->bik", old_rotmat, new_rotmat
    )  # [b,3,3]
    body_param_dict["transf_transl"] = (
        torch.einsum("bij,btj->bti", old_rotmat, new_transl) + old_transl
    )  # [b,1,3]
...
def transform_local_points_to_global(local_points, transf_rotmat, transf_transl):
    ...
    """
    global_points = (
        torch.einsum("bij,bkj->bki", transf_rotmat, local_points) + transf_transl
    )
...
def transform_global_points_to_local(global_points, transf_rotmat, transf_transl):
    ...
    local_points = torch.einsum(
        "bij,bki->bkj", transf_rotmat, global_points - transf_transl
    )
    return local_points
...
class PrimitiveUtility:
    def __init__(
        ...
        # Only load the requested gender
        self.bm_male = bm
        self.bm_female = bm
    ...
    def feature_dict_to_smpl_dict(self, feature_dict):
        ...
        body_pose = transforms.rotation_6d_to_matrix(body_pose).reshape(
            *prefix_shape, 21, 3, 3
        )
        body_param["global_orient"] = global_orient
        body_param["body_pose"] = body_pose
    ...
    def smpl_dict_to_vertices(self, body_param):
        ...
        vertices = body_model(
            betas=body_param["betas"].reshape(B * T, 10),
    ...
    def smpl_dict_inference(self, body_param, return_vertices=False, batch_size=512):
        ...
        while batch_start_idx < T:
            ...
            smplx_out = body_model(
                betas=body_param["betas"][batch_start_idx:batch_end_idx],
                global_orient=body_param["global_orient"][
    ...
    def get_new_coordinate(
        self, body_param_dict, use_predicted_joints=False, pred_joints=None
    ):
        if use_predicted_joints:
            joints = pred_joints
        else:
            body_model = (
                self.bm_male if body_param_dict["gender"] == "male" else self.bm_female
            )
            joints = body_model(**body_param_dict).joints  # [b,J,3]

        new_rotmat, new_transl = get_new_coordinate(
            joints
        )  # transformation from new coord axis to old coord axis
    ...
    def canonicalize(self, primitive_dict, use_predicted_joints=False):
        ...
        delta_T = (
            primitive_dict["pelvis_delta"]
            if "pelvis_delta" in primitive_dict
            else self.calc_calibrate_offset(body_param_dict)
        )  # [b,3]
        transf_rotmat, transf_transl = self.get_new_coordinate(
            body_param_dict,
            use_predicted_joints=use_predicted_joints,
            pred_joints=primitive_dict["joints"][:, 0, :].reshape(-1, 22, 3)
        ...
        if "joints" in primitive_dict:
            ...
            joints = primitive_dict["joints"].reshape(
                B, T, 22, 3
            )  # [b, T, 22*3] -> [b, T, 22, 3]
            joints = torch.einsum(
                "bij,btkj->btki",
                transf_rotmat.permute(0, 2, 1),
                joints - transf_transl.unsqueeze(1),
            )
    ...
    def calc_features(self, primitive_dict, use_predicted_joints=False):
        ...
        if use_predicted_joints:
            ...
            body_model = (
                self.bm_male if primitive_dict["gender"] == "male" else self.bm_female
            )
            smplx_out = body_model(
                betas=primitive_dict["betas"].reshape(B * T, 10),
    ...
    def get_blended_feature(self, feature_dict, use_predicted_joints=False):
        ...
        transf_rotmat, transf_transl, primitive_dict = self.canonicalize(
            primitive_dict, use_predicted_joints=use_predicted_joints
        )
        if use_predicted_joints:  #  directly use the predicted joints, no blending
            ...
            global_orient_rotmat = transforms.rotation_6d_to_matrix(
                global_orient_6d
            )  # [b, T, 3, 3]
            global_orient_rotmat = torch.matmul(
                transf_rotmat.permute(0, 2, 1).unsqueeze(1), global_orient_rotmat
            )
            global_orient_6d = transforms.matrix_to_rotation_6d(
                global_orient_rotmat
            )  # [b, T, 6]
            new_poses_6d = torch.cat(
                [global_orient_6d, poses_6d[:, :, 6:]], dim=-1
            )  # [b, T, 22*6]
            global_orient_delta_6d = feature_dict[
                "global_orient_delta_6d"
            ]  # [b, T, 6], not change
            ...
            global_orient_delta_6d = transforms.matrix_to_rotation_6d(
                global_orient_delta_rotmat
            )  # [b, T, 6]
            transl = primitive_dict[
                "transl"
            ]  # [b, T, 3], from canonicalized primitive dict
            joints = primitive_dict[
                "joints"
            ]  # [b, T, 22*3], from canonicalized primitive dict
            transl_delta = feature_dict["transl_delta"]  # [b, T, 3]
            joints_delta = feature_dict["joints_delta"].reshape(
                B, T, 22, 3
            )  # [b, T, 22*3]
            transl_delta = torch.einsum(
                "bij,btj->bti", transf_rotmat.permute(0, 2, 1), transl_delta
            )  # [b,3]
            joints_delta = torch.einsum(
                "bij,btkj->btki", transf_rotmat.permute(0, 2, 1), joints_delta
            ).reshape(B, T, 22 * 3)
            smpl_features = {
                "transl": transl,
            ...
            last_global_orient_delta_6d = feature_dict["global_orient_delta_6d"][
                :, -1, :
            ]  # [b,6], not change
            last_global_orient_delta_rotmat = transforms.rotation_6d_to_matrix(
                last_global_orient_delta_6d
            )  # [b,3,3]
            last_global_orient_delta_rotmat = torch.matmul(
                torch.matmul(
                    transf_rotmat.permute(0, 2, 1), last_global_orient_delta_rotmat
                ),
                transf_rotmat,
            )  # [b,3,3]
            last_global_orient_delta_6d = transforms.matrix_to_rotation_6d(
                last_global_orient_delta_rotmat
            )  # [b,6]
            # transform the last frame delta features
            last_transl_delta = torch.einsum(
                "bij,bj->bi", transf_rotmat.permute(0, 2, 1), last_transl_delta
            )  # [b,3]
            last_joints_delta = torch.einsum(
                "bij,bkj->bki",
                transf_rotmat.permute(0, 2, 1),
                last_joints_delta.reshape(-1, 22, 3),
            ).reshape(-1, 22 * 3)  # [b,22*3]

            smpl_features["transl_delta"] = torch.cat(
                [smpl_features["transl_delta"], last_transl_delta.unsqueeze(1)], dim=1
            )
            smpl_features["joints_delta"] = torch.cat(
                [smpl_features["joints_delta"], last_joints_delta.unsqueeze(1)], dim=1
            )
            smpl_features["global_orient_delta_6d"] = torch.cat(
                [
                    smpl_features["global_orient_delta_6d"],
                    last_global_orient_delta_6d.unsqueeze(1),
                ],
                dim=1,
            )
    ...
    def transform_feature_to_world(self, feature_dict, use_predicted_joints=True):
        ...
        global_orient_rotmat = torch.matmul(
            transf_rotmat.unsqueeze(1), global_orient_rotmat
        )
        global_orient_6d = transforms.matrix_to_rotation_6d(
            global_orient_rotmat
        )  # [b, T, 6]
        ...
        transl = (
            torch.einsum("bij,btj->bti", transf_rotmat, transl + delta_T.unsqueeze(1))
            - delta_T.unsqueeze(1)
            + transf_transl
        )
        joints = torch.einsum(
            "bij,btkj->btki", transf_rotmat, joints
        ) + transf_transl.unsqueeze(1)
        joints = joints.reshape(B, T, 22 * 3)
        transl_delta = feature_dict["transl_delta"]  # [b, T, 3]
        joints_delta = feature_dict["joints_delta"].reshape(B, T, 22, 3)  # [b, T, 22*3]
        transl_delta = torch.einsum(
            "bij,btj->bti", transf_rotmat, transl_delta
        )  # [b,3]
        joints_delta = torch.einsum(
            "bij,btkj->btki", transf_rotmat, joints_delta
        ).reshape(B, T, 22 * 3)

        world_feature_dict = {
            "transf_rotmat": torch.eye(3, device=device, dtype=dtype)
            .unsqueeze(0)
            .repeat(batch_size, 1, 1),
            "transf_transl": torch.zeros(3, device=device, dtype=dtype)
            .reshape(1, 1, 3)
            .repeat(batch_size, 1, 1),
            "gender": feature_dict["gender"],
            "betas": feature_dict["betas"],
    ...
    def transform_primitive_to_world(self, primitive_dict):
        delta_T = (
            primitive_dict["pelvis_delta"]
            if "pelvis_delta" in primitive_dict
            else self.calc_calibrate_offset(
                {
                    "gender": primitive_dict["gender"],
                    "betas": primitive_dict["betas"][:, 0, :],
                }
        ...
        primitive_dict["transf_rotmat"] = (
            torch.eye(3)
            .unsqueeze(0)
            .repeat(transf_rotmat.shape[0], 1, 1)
            .to(device=self.device, dtype=self.dtype)
        )
        primitive_dict["transf_transl"] = torch.zeros(transf_transl.shape).to(
...
FOOT_JOINTS_IDX = [
    JOINT_NAMES.index(joint_name) for joint_name in ["left_foot", "right_foot"]
]

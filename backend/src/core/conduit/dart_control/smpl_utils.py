...
body_model_dir = Path(__file__).resolve().parents[4] / 'assets' / 'dart_control' / 'smplx_models'
...
class PrimitiveUtility:
    def __init__(self, device='cpu', dtype=torch.float32, motion_repr=None, body_type='smplx', gender='male'):
        self.device = device
        self.dtype = dtype
        self.motion_repr = {
            'transl': 3,
        ...
        bm = smplx.build_layer(str(body_model_dir), model_type=body_type,
                                gender=gender, ext='npz',
                                num_pca_comps=12).to(self.device).eval()

...
class DiscordIO(Component[[Channel[AudioFrame], Channel[InterruptFrame]], DiscordOutputs]):
    ...
    def __init__(self, config: DiscordConfig | None = None) -> None:
        ...
        if not self.token:
            raise ValueError("Discord token must be provided in config or DISCORD_TOKEN env var")
    ...
    def _register_handlers_for_bot(self, bot: discord.Bot) -> None:
        ...
        async def join(ctx: discord.ApplicationContext):
            ...
            sink = _DiscordSink(self._output_audio, ring, self.config.sample_rate, self.config.channels)
...
class _DiscordSink(Sink):
    def __init__(self, output: Channel[AudioFrame], ring: deque[bytes], sr: int, ch: int):
        super().__init__()
        self.output = output
        self.ring = ring
        self.sr = sr
    ...
    def write(self, data: bytes, user):
        ...
        self.output.send(AudioFrame(display_name="discord_audio", data=data, sample_rate=self.sr, channels=self.ch))

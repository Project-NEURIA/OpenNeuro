...
async def tts(req: Annotated[ServeTTSRequest, Body(exclusive=True)]):
    ...
    try:
        ...
        # Resolve reference_id from preloaded references
        # If the reference_id exists in our preloaded map, we inject the references 
        # and NULLify the reference_id in the request so the engine uses our injected objects.
        logger.info(f"Incoming request: reference_id={req.reference_id}, text={req.text[:50]}...")
        if req.reference_id and req.reference_id in model_manager.references:
            logger.info(f"Found preloaded reference for {req.reference_id}")
            if not req.references:
                req.references = model_manager.references[req.reference_id]
            else:
                req.references.extend(model_manager.references[req.reference_id])
            # Clear reference_id so engine doesn't try to look it up in the 'references/' folder
            req.reference_id = None
        elif req.reference_id:
            logger.warning(f"reference_id {req.reference_id} NOT found in preloaded references: {list(model_manager.references.keys())}")
        ...
        try:
            if req.streaming:
                # For streaming, we yield chunks. Each chunk is already int16 bytes.
                ...
                # For non-streaming, we gather all pcm_bytes and then format.
                all_pcm_bytes = []
                ...
                if req.format == "pcm":
                    # Raw PCM return
                    return StreamResponse(
                        iterable=buffer_to_async_generator(combined_pcm),
                        headers={
                            "Content-Disposition": f"attachment; filename=audio.pcm",
                        },
                        content_type=get_content_type("pcm"),
                    )

                # Convert combined int16 bytes back to float32 for soundfile encoding
                audio_array = np.frombuffer(combined_pcm, dtype=np.int16).astype(np.float32) / AMPLITUDE
            ...
            raise HTTPException(
                ...
                content={"statusCode": 500, "message": str(e), "error": "Internal Server Error"}
            )

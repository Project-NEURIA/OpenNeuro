...
def load_model(config_name, checkpoint_path, device="cuda", quantize=False):
    ...
    # Load state dict FIRST before quantizing
    result = model.load_state_dict(state_dict, strict=False, assign=True)
    logger.info(f"Loaded model state dict: {result}")

    if quantize:
        logger.info("Applying 4-bit quantization to DAC decoder...")
        ...
        for name, module in model.named_modules():
            if isinstance(module, nn.Linear):
                ...
                # Copy loaded weights to the 4-bit layer on CPU
                with torch.no_grad():

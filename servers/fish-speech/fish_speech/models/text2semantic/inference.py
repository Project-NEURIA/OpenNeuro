...
def generate(
    ...
    **sampling_kwargs,
    ...
    """
    Generate codes from a prompt, yielding tokens as they are generated.
    """
    T = prompt.size(1)
    max_seq_len = min(max_seq_len, model.config.max_seq_len)
    ...
    semantic_logit_bias = torch.full((1, 1, vocab_size), float("-inf"), device=device, dtype=dtype)
    semantic_logit_bias[0, 0, model.config.semantic_begin_id : model.config.semantic_end_id + 1] = 0.0
    ...
    current_token = first_token.view(1, codebook_dim, -1)
    ...
    for _ in range(max_new_tokens - 1):
        ...
        if token[0, 0].item() == model.tokenizer.get_token_id(IM_END_TOKEN):
            break
            
        yield token
        current_token = token.view(1, codebook_dim, -1)
        input_pos += 1



def init_model(checkpoint_path, device, precision, compile=False, load_4bit=False):
    # Enable BitsAndBytes quantization configs for faster loading if possible
    model = DualARTransformer.from_pretrained(checkpoint_path, load_weights=True)

    if load_4bit:
        logger.info("Applying 4-bit quantization to entire AR model...")
        from bitsandbytes.nn import Linear4bit
        import torch.nn as nn

        # Define which layers to quantize (AR layers)
        # Create a static list to avoid "dictionary changed size during iteration"
        modules_to_quantize = [
        ...
        for name, module in modules_to_quantize:
            # Apply 4-bit to all linear layers (Slow and Fast AR)
            parent = model
            parts = name.split(".")
            for part in parts[:-1]:
            ...
            old_layer = module
            # Create 4-bit layer - we'll let it stay on CPU initially
            # but ensure its weight is marked as Params4bit properly
            ...
            # Copy data to the Linear4bit weight while it is still on CPU.
            # This avoids the "size mismatch" during .copy_() after moving to CUDA.
            with torch.no_grad():
                new_layer.weight.data.copy_(old_layer.weight.data)
                if old_layer.bias is not None:
                    new_layer.bias.data.copy_(old_layer.bias.data)
            
            # Move to device - this triggers the actual quantization and packing
            new_layer = new_layer.to(device)
            
            setattr(parent, parts[-1], new_layer)
        logger.info("4-bit quantization applied to AR model.")

    # Move non-quantized parts of the model to device
...
def generate_long(
    *,
    ...
    # New streaming hyperparams
    streaming_chunk_size: int = 30,
    streaming_overlap_size: int = 10,
    max_seq_len: int = 4096,
):
    ...
    for sample_idx in range(num_samples):
        ...
        for batch_idx, batch_text in enumerate(batches):
            ...
            y_list = []
            current_chunk = []
            # Overlap/Lookahead Settings:
            chunk_size = streaming_chunk_size
            overlap_size = streaming_overlap_size

            for y_token in generate(
                model=model,
                prompt=encoded,
                ...
                current_chunk.append(y_token)
                
                if len(current_chunk) >= chunk_size:
                    # To produce a smooth segment, we send [overlap + chunk_size]
                    # tokens to the decoder so it has context for the boundary.
                    context_start = max(0, len(y_list) - len(current_chunk) - overlap_size)
                    chunk_tensor = torch.cat(y_list[context_start:], dim=1)
                    
                    # Store the actual number of new tokens we want from this result
                    action_meta = {
                        "new_tokens": len(current_chunk),
                        "overlap": len(y_list) - len(current_chunk) - context_start
                    }
                    
                    yield GenerateResponse(action="sample", codes=chunk_tensor, text=str(action_meta))
                    current_chunk = []
...
def launch_thread_safe_queue(
    ...
    def worker():
        ...
        with torch.device(device):
            model.setup_caches(
                max_batch_size=1,
                max_seq_len=min(4096, model.config.max_seq_len),
                dtype=next(model.parameters()).dtype,
            )
        init_event.set()
        ...
        while True:
            ...
            try:
                # No longer need generate_long to return a list
                ...
                # Send terminal signal if not already handled by actions
                # (TTSInferenceEngine loop expects action="next" to stop)
                if torch.cuda.is_available():

...
class VQManager:
    ...
    def decode_vq_tokens(self, codes):
        ...
        if isinstance(self.decoder_model, DAC):
            # Ensure codes are (num_codebooks, T)
            if codes.ndim == 3 and codes.shape[0] == 1:
                codes = codes.squeeze(0)
            
            # Fish Speech S2 Pro uses 11 codebooks (1 semantic + 10 acoustic)
            # The DAC model expects exactly 10 acoustic codebooks for decode.
            # We strip the first codebook (semantic) before passing to DAC.
            if codes.shape[0] == 11:
                codes = codes[1:]
